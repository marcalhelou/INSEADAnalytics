...
**Your Answers here:**
1. In the file dataSet1.R, it uses the tickers SPY and AAPL. The second is Apple so it gives the stock retuns of apple in the second column
2. cumulat = round(100*sum(StockReturns[,2]),1); 'cumulat = 596.4'
   avgret = round(100*mean(StockReturns[,2]),1); 'avgret = 0.1'
   stddev = round(100*sd(StockReturns[,2]),3); 'stddev = 2.39'
3. You have to change the ticker in dataSet1.R to include yahoo to load the data and calculate the statistics
<hr>
<hr>
...
1. nrow(transposedData)
   ncol(transposedData)
2. portfolio = apply(StockReturns,1,mean)
   pnl_plot(portfolio)
<hr>
<hr>
...
**Your Answers here:**
1. Change the starting date to 'startDate = "2000-12-29". The first working day is January 2 so that's when our data starts'
2. Change the tickers to the following'mytickers = c("SPY", "AAPL","GS","TSLA")' and the starting date in dataSet1.R and save the file
   portfolio = apply(StockReturns,1,mean)
   pnl_plot(portfolio)
...
**Your Answers here:**
1. For every row, it compares myData with StockReturns, and, because they are different(class and dimension), the binary gives 1, which summed over 20 rows gives an answer of 20
2. It will not work since we are adding matrices with different dimensions. Also class(myData)="data.frame" while class(StockReturns)="matrix" which means they are different classes and thus can't be added
